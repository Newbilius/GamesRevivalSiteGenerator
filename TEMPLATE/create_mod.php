    ...
    foreach ($values as $key => $value) {
        if (strlen($value) < 5) {
            ...
            $values[$key] = str_replace(["\\n", "\\r"], '',
                $values[$key]); //двойные слеши не баг, а фича, специфика косяка где-то в движке генерации страницы
        }
    }
...
function translit($text)
{
    $cyr = [
        ...
        'я', 'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И',
        'Й', 'К', 'Л', 'М', 'Н', 'О', 'П', 'Р', 'С', 'Т', 'У', 'Ф', 'Х', 'Ц', 'Ч', 'Ш', 'Щ',
        'Ъ', 'Ы', 'Ь', 'Э', 'Ю', 'Я',
    ];
    $lat = [
        'a', 'b', 'v', 'g', 'd', 'e', 'io', 'zh', 'z', 'i', 'y', 'k', 'l', 'm',
        'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'h', 'ts', 'ch', 'sh', 'sht', 'a', 'i', 'y', 'e',
        'yu', 'ya', 'A', 'B', 'V', 'G', 'D', 'E', 'Io', 'Zh', 'Z', 'I', 'Y', 'K', 'L', 'M', 'N',
        'O', 'P', 'R', 'S', 'T', 'U', 'F', 'H', 'Ts', 'Ch', 'Sh', 'Sht', 'A', 'I', 'Y', 'e', 'Yu', 'Ya',
    ];
...
if (!empty($_POST['formPosted'])) {
    ...
    if (empty($errors)) {
        ...
        //скриншоты
        $modImagesNames = ["modFilePics1", "modFilePics2", "modFilePics3", "modFilePics4"];
